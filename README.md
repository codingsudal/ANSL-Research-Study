...
> **학부연구생 지원을 위한 논문 리뷰 및 네트워크 학습 저장소**
...
* **핵심 가설:** 'Sink Token'과 직교하는(Orthogonal) 토큰일수록 고유한 정보를 가진 중요한 데이터일 것이다.

### 3. 연구 방법 
* 각 레이어에서 토큰과 Sink Token 사이의 코사인 유사도(Cosine Similarity)를 측정.
* 유사도가 낮은 토큰을 '중요 토큰'으로 판별하여 다음 연산에 우선적으로 참여시킴.

### 4. 핵심 결과 
* **높은 정확도 유지:** 동일한 연산량 절감(Sparsity) 조건에서 기존 레이어 푸닝(Layer Pruning) 방식보다 압도적인 정확도를 보임.
* **긴 데이터셋 강점:** 문장이 매우 긴 데이터셋(LongBench)에서도 성능 저하 없이 효율적인 추론 가능.
